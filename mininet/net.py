...
class Mininet(object):
            ...
            nodes = (self.hosts if node in self.hosts else
                     (self.switches if node in self.switches else
                      (self.controllers if node in self.controllers else
                       (self.tempSwitches if node in self.tempSwitches else
                        []))))
    ...
    def addSwitch(self, name, cls=None, **params):
        ...
        ############################### Meghana change start ###############################
        # Adding temporary switch
        if name[0:5] == 'stemp':
            tempsw = cls(name, **defaults)
            self.tempSwitches.append(tempsw)
            self.nameToNode[name] = tempsw
            return tempsw
    ...
    def delSwitch(self, switch):
        """Delete a switch.
            ...
            tempSwitches: name of temporary switch"""
        self.delNode(switch, nodes=self.switches)
        ############################### Meghana change start ###############################
        self.delNode(switch, nodes=self.tempSwitches)
        ############################### Meghana change end ###############################
    ...
    ############################### Meghana change start ###############################
    def addQSpec(self, queue, priority, maxrate, **params):
    ...
    def addQlink(self, node1, node2, params1, params2, qspec1=None, qspec2=None):
        ...
        if str(addlink1.intf1)[0] == 's':
            self.qintf.append(addlink1.intf1)
            self.qparams[addlink1.intf1] = [qspec1, bw1] # add qparam to the interface dict.
            node1.addQlink_has_been_called = True
        ...
        if str(addlink2.intf2)[0] == 's':
            ...
            node2.addQlink_has_been_called = True

        # Run temp switch in standalone mode
        for tsw in self.tempSwitches:
    ...
    def __qosSwitch(self, node, intf):
        ...
        for qspec in qspecs:
            ...
            if 'priority' in qspec:
                ...
                if priority and (priority < 0 or priority > maxpriority):
            ...
            if 'maxrate' in qspec:
                ...
                maxrate = 0
            maxrate_in_bytes = maxrate*bw*1000000
            maxrates.append(int(maxrate_in_bytes))
            ...
            if 'minrate' in qspec:
                ...
                minrate = 0
            minrate_in_bytes = minrate*bw*1000000
            minrates.append(int(minrate_in_bytes))
        ...
        for tswclass, tswitches in groupby(
            ...
            for switch in self.switches:
                for intf in switch.intfList():
                    if switch.addQlink_has_been_called == True:
                        if intf in self.qintf:
                            self.__qosSwitch(switch, intf)

            info('*** Add flows on switch interfaces ***')
            self.__addFlow()
    ...
    def stop(self):
        ...
        for switch in self.switches:
            if switch.addQlink_has_been_called:
